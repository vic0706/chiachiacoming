  ...
  return (
    <div className="h-full overflow-y-auto px-4 py-6 space-y-8 animate-fade-in no-scrollbar pb-24">
      <section>
        <div className="flex justify-between items-center mb-4">
          ...
          {upcomingRaces.hasMore && (
            <button onClick={onNavigateToRaces} className="text-[10px] text-chiachia-green font-black tracking-widest uppercase flex items-center gap-1">
            ...
            upcomingRaces.topTwo.map((race, idx) => (
              <div key={idx} className="rounded-2xl p-4 flex items-center gap-4 relative overflow-hidden group border border-chiachia-green/40 shadow-[0_4px_15px_rgba(57,231,95,0.15)] bg-zinc-900/40 backdrop-blur-md">
                 ...
                 <div className="flex-none flex flex-col items-center justify-center bg-black/70 border border-white/20 rounded-xl w-12 h-12 relative z-10 shadow-lg">
                    <span className="text-[8px] text-chiachia-green uppercase font-black">{format(race.dateObj, 'MMM')}</span>
                 ...
                 <div className="flex-1 min-w-0 relative z-10">
                   ...
                   <div className="text-[10px] text-chiachia-green font-black uppercase tracking-wider mt-1 drop-shadow-sm">{race.race_group || 'BxB'}</div>
                 </div>
                 {race.address && (
                   <button 
                     onClick={(e) => { e.stopPropagation(); handleNavigate(race.address); }} 
                     className="p-2 rounded-xl bg-black/60 text-white border border-white/20 backdrop-blur-md active:scale-90 transition-all z-10 shadow-liquid hover:bg-chiachia-green/20"
                   >
      ...
      {selectedChartType && activeAges.length > 0 && (
            <div className="w-full relative overflow-hidden h-24 -mt-4 mb-[-50px]">
                            ...
                            <div key={`${age}-${i}`} className="flex items-center gap-4 shrink-0">
                                ...
                                <div className="flex flex-col items-end">
                                    <span className="text-[10px] font-black text-zinc-400 tracking-wider mb-0.5 italic flex items-center">
                                       <span className="text-chiachia-green not-italic mr-1.5 bg-chiachia-green/10 px-1 rounded shadow-sm">{age}歲</span>最速伝説
                                    </span>
                                    {/* Gold Glory Effect */}
                                    <span className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-b from-yellow-300 to-yellow-600 font-mono italic tracking-tighter leading-none drop-shadow-[0_0_8px_rgba(251,191,36,0.5)]">
                                        {info.value.toFixed(3)}s
                                    </span>
                                </div>
                                
                                {/* Right: Avatar */}
                                <div className="w-16 h-16 rounded-full border-2 border-yellow-400/80 shadow-[0_0_15px_rgba(251,191,36,0.4)] relative overflow-hidden bg-zinc-900 flex items-center justify-center shrink-0">
                                   {avatar}
      ...
      <section className="mt-4">
          <div className="mb-2">
              <h2 className="text-[10px] font-black text-zinc-500 tracking-[0.2em] uppercase flex items-center gap-1.5">
                  <Activity size={12} className="text-chiachia-green" /> 訓練趨勢分析
              </h2>
          </div>
          ...
          <div className="flex flex-col gap-2 mb-4">
                ...
                <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="relative group flex-1 h-9">
                        ...
                        <select 
                            ...
                            className="w-full h-full appearance-none bg-transparent text-white font-black text-xs rounded-xl pl-3 pr-8 outline-none text-left"
                        >
                    ...
                    {/* Date Buttons */}
                    <div className="flex items-center bg-zinc-900/50 p-1 rounded-xl border border-white/5 gap-1 shrink-0 h-9">
                        {(['1W', '1M', '3M'] as const).map(range => (
                            <button 
                                key={range}
                                onClick={() => setQuickRange(range)} 
                                className={`text-[9px] font-bold px-2.5 h-full rounded-lg transition-all ${activeRange === range ? 'bg-white/10 text-white shadow-inner' : 'text-zinc-500 hover:text-zinc-300'}`}
                            >
                                {range}
                            </button>
                        ))}
                        ...
                        <button 
                            ...
                            className={`px-2 h-full rounded-lg transition-all flex items-center justify-center ${activeRange === 'custom' ? 'bg-chiachia-green/20 text-chiachia-green' : 'text-zinc-500 hover:text-zinc-300'}`}
                        >
                            <Calendar size={14} />
                        </button>
                    </div>
                ...
                {showCustomDate && (
                    <div className="flex items-center justify-end gap-2 animate-fade-in bg-zinc-900/30 p-2 rounded-xl border border-white/5 border-dashed">
                        ...
                        <input type="date" value={startDate} onChange={(e) => { setStartDate(e.target.value); setActiveRange('custom'); }} className="bg-zinc-950 text-[10px] font-mono text-zinc-300 outline-none w-auto min-w-[80px] text-center p-1 rounded border border-white/10" />
                        <span className="text-zinc-600 text-[10px]">-</span>
                        <input type="date" value={endDate} onChange={(e) => { setEndDate(e.target.value); setActiveRange('custom'); }} className="bg-zinc-950 text-[10px] font-mono text-zinc-300 outline-none w-auto min-w-[80px] text-center p-1 rounded border border-white/10" />
                    </div>
                )}
          </div>
            ...
            <div className="space-y-6">
                 {currentTypeSummaries.length > 0 ? currentTypeSummaries.map((summary, idx) => {
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                             <ComposedChart data={chartData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }} barGap={2}>
                                ...
                                <YAxis 
                                    yAxisId="right"
                                    orientation="right"
                                    tick={{fontSize: 9, fill: '#39e75f', fontFamily: 'monospace'}} 
                                    axisLine={false} 
                                    tickLine={false} 
                                    domain={[0, 100]}
                                    hide={false}
                     ...
                     <div className="py-12 flex flex-col items-center justify-center text-zinc-600">
                         ...
                         <span className="text-[10px] font-black uppercase tracking-widest">在此區間無數據</span>
                     </div>
                 )}
            </div>
          ) : (
              <div className="py-20 text-center flex flex-col items-center">
                  ...
                  <p className="text-zinc-500 text-xs font-black uppercase tracking-widest">請選擇訓練項目以查看分析</p>
              </div>
          )}
      </section>
               ...
               <div className="flex-1 min-h-0 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                            ...
                            <XAxis 
                                dataKey="name" 
                                tick={{fontSize: 12, fill: '#fff', fontWeight: 700}} 
                                axisLine={false} 
                                tickLine={false}
                                dy={10}
                                interval={0}
                            ...
                            {/* Right Y-Axis: Stability % */}
                            <YAxis 
                                yAxisId="right"
                                orientation="right"
                                tick={{fontSize: 10, fill: '#39e75f'}} 
                                axisLine={false} 
                                tickLine={false} 
                                domain={[0, 100]}
                            />
                       ...
                       <div key={idx} onClick={() => handleDrillDown(item.name)} className="flex items-center justify-between p-3 rounded-xl bg-zinc-900/50 border border-white/5 active:scale-[0.98] transition-all">
                           ...
                           <div className="flex items-center gap-4">
                               <div className="text-right">
                                   <div className="text-[8px] text-zinc-500 uppercase font-bold">Best</div>
                                   <div className="text-sm font-mono font-black text-white">{item['最快']}s</div>
                               </div>
                               <div className="text-right">
                                   <div className="text-[8px] text-zinc-500 uppercase font-bold">Stability</div>
                                   <div className={`text-sm font-mono font-bold ${item.stability >= 80 ? 'text-emerald-500' : 'text-amber-500'}`}>{item.stability}%</div>
                               </div>
          ...
          <div className="fixed inset-0 z-[110] flex items-end justify-center bg-black/60 backdrop-blur-sm animate-fade-in" onClick={() => setDrillDownData(null)}>
              <div className="glass-card w-full max-w-md rounded-t-[32px] p-6 shadow-2xl animate-slide-up bg-[#1a1a1a] border-white/10 max-h-[70vh] flex flex-col" onClick={e => e.stopPropagation()}>
                  <div className="flex justify-between items-center mb-4 shrink-0">
                       <h3 className="text-lg font-black text-white">{drillDownData.title}</h3>
                       <button onClick={() => setDrillDownData(null)} className="p-2 bg-white/5 rounded-full"><X size={16} /></button>
                  </div>
                  <div className="flex-1 overflow-y-auto no-scrollbar space-y-2">
                      {drillDownData.records.map((rec, i) => (
                          <div key={i} className="flex justify-between items-center p-3 bg-black/40 rounded-xl border border-white/5">
                              ...
                              <span className="text-xl font-mono font-black text-white">{parseFloat(rec.value).toFixed(3)}s</span>
                          </div>
                      ))}
                  </div>
              </div>
