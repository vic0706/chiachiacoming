...
const Personal: React.FC<PersonalProps> = ({ data, people, trainingTypes, refreshData, activePersonId, onSelectPerson, raceGroups, targetDate, onClearTargetDate }) => {
  ...
  const toggleRaceCustom = () => {
      if (raceActiveRange !== 'custom') {
          setRaceActiveRange('custom');
          setRaceShowCustom(true);
      } else {
          setRaceShowCustom(!raceShowCustom);
      }
  };
  ...
  const displayedRaces = useMemo(() => {
      ...
      result = result.filter(r => r.date >= raceStartDate && r.date <= raceEndDate);

      if (raceSearchTerm) {
          const lowerTerm = raceSearchTerm.toLowerCase();
          result = result.filter(r => r.name.toLowerCase().includes(lowerTerm));
      }

      return result.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }, [data, person, raceFilterStatus, raceFilterSeries, raceStartDate, raceEndDate, raceSearchTerm]);
  ...
  const allTimeBest = useMemo(() => {
     ...
     const allRecs = data.filter(d => String(d.people_id) === String(person.id) && d.item === 'training' && (!selectedType || d.name === selectedType));
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.files && e.target.files[0]) {
          setIsUploading(true);
          const file = e.target.files[0];
          const idPart = editingRaceId || Date.now();
          const customName = `race_note_${person?.id}_${idPart}`;
          
          const result = await uploadImage(file, 'race', customName);
          
          if (result.url) {
              const timestampUrl = `${result.url}?t=${Date.now()}`;
              setRaceForm(prev => ({ ...prev, url: timestampUrl }));
              setZoomScale(1);
              setPosX(50);
              setPosY(50);
  ...
  const handleSubmitRace = async () => {
      ...
      const submitData: Partial<DataRecord> = {
          item: 'race',
          date: raceForm.date,
          name: raceForm.name,
          race_id: raceForm.race_id, 
          address: raceForm.address,
          value: raceForm.rank,
          note: raceForm.note,
          url: finalUrl,
          people_id: person.id,
      ...
      if (editingRaceId) {
          submitData.id = editingRaceId;
      }

      const success = await api.submitRecord(submitData);
      if (success) {
          setIsAddingRace(false);
          setEditingRaceId(null);
          await refreshData();
  ...
  const handleEditRace = (race: DataRecord) => {
      ...
      setRaceForm({
          ...
          date: race.date,
          name: race.name,
          race_id: race.series_id || race.race_id || '', 
      ...
      setEditingRaceId(race.id || null);
      setIsAddingRace(true);
  };

  const handleJoinRace = (previewRace: DataRecord) => {
      setRaceForm({
          id: '',
          date: previewRace.date,
          name: previewRace.name,
          race_id: previewRace.series_id || previewRace.race_id || '',
          address: previewRace.address,
          rank: '', 
          note: '',
          url: '',
          event_id: previewRace.event_id || ''
      });
      setZoomScale(1);
      setPosX(50);
      setPosY(50);
  ...
  if (!person) {
      return (
        <div className="h-full flex flex-col items-center justify-center p-10 opacity-50 bg-[#0a0508]">
           ...
           <div className="text-center text-xs font-bold text-zinc-500 tracking-widest uppercase mb-4">
  ...
  return (
    <div className="h-full overflow-y-auto animate-fade-in no-scrollbar pb-32 relative bg-[#0a0508] overflow-x-hidden">
        {/* Hero Section */}
                            ...
                            <>
                                ...
                                <div className="flex flex-col gap-2 mb-2">
                                    ...
                                    <div className="flex gap-2 h-9">
                                        <div className="flex items-center bg-zinc-900/80 p-1 rounded-xl border border-white/5 gap-1 shrink-0">
                                            {(['1W', '1M', '3M'] as const).map(range => (
                                        ...
                                        <div className="flex-1 bg-zinc-900 rounded-xl p-1 flex relative border border-white/5">
                                            {(['registered', 'available', 'finished'] as const).map((status) => (
                                                <button key={status} onClick={() => setRaceFilterStatus(status)} className={`flex-1 text-[9px] font-bold rounded-lg transition-all ${raceFilterStatus === status ? 'bg-zinc-800 text-white shadow-md' : 'text-zinc-500 hover:text-zinc-300'}`}>{status === 'registered' ? '已報名' : status === 'available' ? '可報名' : '已結束'}</button>
                                            ))}
                                        </div>
                                    </div>
                                    {raceShowCustom && (
                                        <div className="flex items-center justify-end gap-2 animate-fade-in bg-zinc-900/30 p-2 rounded-xl border border-white/5 border-dashed">
                                            <span className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider mr-1">Range</span>
                                            <input type="date" value={raceStartDate} onChange={(e) => { setRaceStartDate(e.target.value); setRaceActiveRange('custom'); }} className="bg-zinc-950 text-[10px] font-mono text-zinc-300 outline-none w-auto min-w-[80px] text-center p-1 rounded border border-white/10" />
                                            <span className="text-zinc-600 text-[10px]">-</span>
                                            <input type="date" value={raceEndDate} onChange={(e) => { setRaceEndDate(e.target.value); setRaceActiveRange('custom'); }} className="bg-zinc-950 text-[10px] font-mono text-zinc-300 outline-none w-auto min-w-[80px] text-center p-1 rounded border border-white/10" />
                                        </div>
                                    )}
                                </div>
                                                    ...
                                                    <div className="flex flex-col items-end gap-1">
                                                        <div className="flex gap-2 mb-2 animate-fade-in" onClick={e => e.stopPropagation()}>
                                                            ...
                                                            {race.address && isExpanded && (
                                                                <button onClick={() => { if(race.address) window.open(race.address.startsWith('http') ? race.address : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(race.address)}`, '_blank'); }} className="p-2 rounded-xl bg-black/60 text-white border border-white/20 backdrop-blur-md active:scale-90 shadow-lg">{race.address.startsWith('http') ? <ExternalLink size={14} /> : <MapPin size={14} />}</button>
                                                            )}
                                                        </div>
                                                        <div className="flex flex-col items-end">
                                                            {isPreview ? (
                                                                <button onClick={(e) => { e.stopPropagation(); handleJoinRace(race); }} className="flex flex-col items-center justify-center gap-1 bg-emerald-600 hover:bg-emerald-700 rounded-xl p-2.5 transition-all active:scale-95 group/join shadow-lg shadow-emerald-900/50 border border-white/10">
                                                                ...
                                                                <button onClick={(e) => { e.stopPropagation(); handleWithdrawRace(race.id!); }} className="flex flex-col items-center justify-center gap-1 bg-rose-600 hover:bg-rose-700 rounded-xl p-2.5 transition-all active:scale-95 group/withdraw shadow-lg shadow-rose-900/50 border border-white/10">
